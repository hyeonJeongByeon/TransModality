import keras
import numpy as np
from keras.layers import TimeDistributed, Softmax, Dense
...
def _wrap_layer(name, input_layer, build_func, dropout_rate=0.0, trainable=True, prefix=""):
    ...
    if dropout_rate > 0.0:
        dropout_layer = keras.layers.Dropout(
            rate=dropout_rate,
            name='{}{}-Dropout'.format(prefix, name),
        )(build_output)
    else:
        dropout_layer = build_output
    if isinstance(input_layer, list):
        input_layer = input_layer[0]
    add_layer = keras.layers.Add(name='{}{}-Add'.format(prefix, name))([input_layer, dropout_layer])
    normal_layer = LayerNormalization(
        trainable=trainable,
        name='{}{}-Norm'.format(prefix, name),
    )(add_layer)
    return normal_layer
...
def double_trans_modality_part(main_input,
                               middle_input,
                               main_dim,
                               middle_dim,
                               encoder_num,
    ...
    first_decoded_output = TimeDistributed(Dense(middle_dim, activation="relu"))(first_decoded_layer)
    ...
    second_decoded_output = TimeDistributed(Dense(main_dim, activation="relu"))(second_decoded_layer)
